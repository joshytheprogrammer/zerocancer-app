...
analyticsApp.get("/dashboard", async (c) => {
  try {
    const db = getDB(c);
    const metrics = await analyticsService.getDashboardMetrics(db);
...
analyticsApp.get(
  ...
  async (c) => {
    try {
      ...
      const db = getDB(c);
      const report = await analyticsService.getTimeBasedReport(db, period, {
        from: startDate,
        to: endDate,
      });
...
analyticsApp.get("/geographic", async (c) => {
  try {
    const db = getDB(c);
    const report = await analyticsService.getGeographicReport(db);
...
analyticsApp.get("/campaigns", async (c) => {
  try {
    const db = getDB(c);
    const analytics = await analyticsService.getCampaignAnalytics(db);
